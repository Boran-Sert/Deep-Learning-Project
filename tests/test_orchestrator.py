...
def test_experiment_orchestrator_events(mock_get_loader, mock_dataset):
    ...
    # Kasıtlı olarak konfigürasyonu ezelim (testin hızlı bitmesi için
    # az epoch ve 1 seed)
    ...
    eval_events = [
        e for e in received_events if isinstance(e, EvaluationCompletedEvent)
    ]
    ...
    assert any("farklı varyansı temsil eder" in e.note for e in cross_events), (
        "Cross dataset note eksik!"
    )
    ...
    assert len(unseen_events) > 0, (
        "AutomataDecisionEvent (Unseen Pattern) fırlatılmadı!"
    )

    # Sensitivity analysis kontrol
    sensitivity_events = [
        e for e in received_events if isinstance(e, SensitivityAnalysisEvent)
    ]
