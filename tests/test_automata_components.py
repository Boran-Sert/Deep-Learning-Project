...
def test_automata_train_transition_matrix():
    ...
    for _current_word, next_words in detector.transition_matrix.items():
        prob_sum = sum(next_words.values())
        assert np.isclose(prob_sum, 1.0), (
            f"Geçiş olasılıkları toplamı 1.0 değil: {prob_sum}"
        )


def test_automata_predict():
    """AutomataDetector'ın predict işlemi sonrası doğru uzunlukta

    ve tipte döndüğünü test eder.
    """
    ...
    assert len(predictions) == test_len, (
        f"Boyut uyumsuzluğu: Expected {test_len}, Got {len(predictions)}"
    )
