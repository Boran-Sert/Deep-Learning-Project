...
class SlidingWindowExtractor:
    ...
    def __init__(self, window_size: Optional[int] = None):
        ...
        self.window_size = (
            ...
            else self.config.get("automata.window_size", 5)
        )

    def extract(
        self, symbols: List[str], source_files: Optional[pd.Series] = None
    ) -> List[Optional[str]]:
        """
        ...
        Args:
            ...
            source_files (pd.Series, optional): Her harfin/satırın ait
                                                olduğu kaynak dosya serisi.

        Returns:
            List[Optional[str]]: Pencere boyutu kadar uzunluktaki kelime
                                 listesi veya None (sınır ihlali varsa).
        """
        words = []
        n = len(symbols)
