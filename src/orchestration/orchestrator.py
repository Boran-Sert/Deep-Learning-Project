...
class ExperimentOrchestrator:
    ...
    Cross-validation (Fold loop) desteği ile tüm verileri fold bazında eğitir ve test eder.
    """
    ...
    def _train_all_models(self, train_df: pd.DataFrame, val_df: Optional[pd.DataFrame], dataset_name: str, fold_idx: int):
        """Eğitimi fold bazlı yapar ve sonuçları kaydeder."""
        pipeline = PreprocessorPipeline(experiment_id=f"exp_{dataset_name}_fold{fold_idx}")
        self._pipelines[fold_idx] = pipeline
        
        X_train_scaled, X_train_pca = pipeline.fit_transform(self._get_features(train_df))
        ...
        for seed in seeds:
            for model_name in ["cnn", "gru"]:
                ...
                adapter = DeepLearningAdapter(model_name=model_name, n_features=X_train_scaled.shape[1])
                ...
                self.event_bus.publish(ModelTrainedEvent(model_name=model_name, dataset_name=dataset_name, seed=seed, fold_idx=fold_idx))
        ...
        self.event_bus.publish(ModelTrainedEvent(model_name="automata", dataset_name=dataset_name, seed=42, fold_idx=fold_idx))

    def _run_original_scenario(self, train_df: pd.DataFrame, test_df: pd.DataFrame, dataset_name: str, fold_idx: int):
        ...
        for seed in seeds:
            for model_name in ["cnn", "gru"]:
                ...
                preds = model.predict(X_test_scaled, source_files_test=source_files_test)
                
                self.event_bus.publish(EvaluationCompletedEvent(
                    scenario="original",
                    train_dataset=dataset_name, test_dataset=dataset_name,
                    model_name=model_name, seed=seed, metrics=compute_metrics(y_test, preds),
                    fold_idx=fold_idx
                ))
        ...
        self.event_bus.publish(EvaluationCompletedEvent(
            scenario="original",
            train_dataset=dataset_name, test_dataset=dataset_name,
            model_name="automata", seed=42, metrics=compute_metrics(y_test, preds),
            fold_idx=fold_idx
        ))

    def _run_noise_scenario(self, train_df: pd.DataFrame, test_df: pd.DataFrame, dataset_name: str, fold_idx: int):
        ...
        for seed in seeds:
            ...
            for model_name in ["cnn", "gru"]:
                key = f"fold_{fold_idx}_{model_name}_seed{seed}"
                model = self._trained_models[key]
                preds = model.predict(X_test_noisy_scaled, source_files_test=source_files_test)
            ...
            automata = self._trained_models[f"fold_{fold_idx}_automata_seed42"]
            preds_auto = automata.predict(X_test_noisy_pca, source_files_test=source_files_test)
            self.event_bus.publish(EvaluationCompletedEvent(
                scenario="noise",
                train_dataset=dataset_name, test_dataset=dataset_name,
                model_name="automata", seed=seed, metrics=compute_metrics(y_test, preds_auto),
                fold_idx=fold_idx
            ))

    def _run_unseen_pattern_scenario(self, train_df: pd.DataFrame, test_df: pd.DataFrame, dataset_name: str, fold_idx: int):
        pipeline = self._pipelines[fold_idx]
        _, X_test_pca = pipeline.transform(self._get_features(test_df))
        y_test = test_df["anomaly"]
        source_files_test = test_df.get("source_file", None)
        
        automata: AutomataDetector = self._trained_models[f"fold_{fold_idx}_automata_seed42"]
        assert automata.vocab_manager is not None
        vocab = automata.vocab_manager.vocabulary
        
        series = X_test_pca.iloc[:, 0].to_numpy()
        test_words = automata._pipeline_transform(series, source_files=source_files_test)
        ...
        unseen_rate = len(unseen_words) / len(test_words_valid) if test_words_valid else 0
        ...
        self.event_bus.publish(AutomataDecisionEvent(
            scenario="unseen_pattern",
            unseen_rate=unseen_rate,
            mapping_log=mapping_log,
            metrics=compute_metrics(y_test, automata.predict(X_test_pca, source_files_test=source_files_test)),
            fold_idx=fold_idx
        ))
    ...
    def _prepare_pc1(self, dataset_name: str):
        ...
        return (X_train_pca, train_df["anomaly"], train_df.get("source_file")), (X_test_pca, test_df["anomaly"], test_df.get("source_file"))
        ...
        for ws in [3, 4, 5, 6]:
            for ab in [3, 4, 5, 6]:
                temp_model = AutomataDetector(
                    experiment_id=f"sens_{ws}_{ab}",
                    window_size=ws,
                    alphabet_size=ab
                )
                temp_model.train(X_train_pca, train_df["anomaly"], source_files_train=sf_tr)
                ...
                nonzero = sum(len(dest) for dest in temp_model.transition_matrix.values())
