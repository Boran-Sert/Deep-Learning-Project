...
class PreprocessorPipeline:
    ...
    Veri sızıntısını (Data Leakage) engellemek amacıyla 'fit' işlemi yalnızca
    Train verisine uygulanır, Scaler ve PCA durumları
    ExperimentArtifactManager üzerinden diske yazılır/okunur.
    """
    ...
    def fit_transform(self, X_train: pd.DataFrame) -> tuple:
        ...
        Returns:
            tuple: (X_train_scaled, X_train_pca)
                - X_train_scaled: Deep Learning modelleri için
                  standartlaştırılmış özellikler.
                - X_train_pca: Automata modeli için tek bileşene (PC1)
                  indirilmiş özellikler.
        """
        ...
        X_train_scaled = pd.DataFrame(
            X_scaled_array, index=X_train.index, columns=X_train.columns
        )
        ...
        X_train_pca = pd.DataFrame(X_pca_array, index=X_train.index, columns=["PC1"])
    ...
    def transform(self, X_test: pd.DataFrame) -> tuple:
        ...
        X_test_scaled = pd.DataFrame(
            X_scaled_array, index=X_test.index, columns=X_test.columns
        )

        X_pca_array = pca.transform(X_test_scaled)

        X_test_pca = pd.DataFrame(X_pca_array, index=X_test.index, columns=["PC1"])
