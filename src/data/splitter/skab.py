...
class SkabGroupFoldStrategy(ISplitStrategy):
    ...
    def split(self, df: pd.DataFrame) -> Generator[Tuple[pd.DataFrame, pd.DataFrame], None, None]:
        ...
        if "source_file" not in df.columns:
            raise ValueError("SkabGroupFoldStrategy gereksinimi olan 'source_file' sütunu bulunamadı.")
        ...
        for train_idx, test_idx in gkf.split(df, groups=groups):
            ...
            # Zaman sırasını garanti altına almak için index'e göre sıralama (genelde sıralıdır ama tedbiren)
