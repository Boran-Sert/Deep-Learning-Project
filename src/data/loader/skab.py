...
class SkabLoader(IDataLoader):
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        if not dataframes:
            raise FileNotFoundError("SKAB veri dosyaları bulunamadı. Lütfen dizin yollarını kontrol edin.")
        ...
        # Zamana göre sırala (her dosyanın kendi içinde zaten sıralı olduğu varsayılır, genel sıralama yapılabilir)
        # Ancak farklı valflerde çakışan zamanlar olabilir, source bazında sıralamak daha iyi olabilir.
        df_combined.sort_index(inplace=True)
        
        return df_combined

    def _read_valve_directory(self, dir_path: str, group_name: str, df_list: List[pd.DataFrame]) -> None:
        ...
        for file_path in csv_files:
            ...
            # SKAB dosyaları genellikle index='datetime' ve ayrıştırıcı olarak sep=';' ile gelir.
            # Virgülle ayrılmış CSV is default, deniyoruz.
            try:
                df = pd.read_csv(file_path, index_col='datetime', parse_dates=True, sep=';')
            except ValueError:
